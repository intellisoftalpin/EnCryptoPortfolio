...
class DatabaseProvider implements DbRepository {
  ...
  dbMigration(String name, String password) async {
   ...
   // var version = await _database?.getVersion();
    Database newDB = await openDatabase(path, password: password, version: 1, onCreate: initDatabase);
 ...
 Future<CoinEntity> getCoin(String id) async {
    var result = await _database!.query(coinTable, where: "coinId = ?", whereArgs: [id]);
    CoinEntity? coin = result.isNotEmpty ? CoinEntity.fromDatabaseJson(result.first) : null;
    return coin!;
  }
  ...
  updateCoinIsRelevant(CoinEntity newCoinEntity) async {
    var result = await _database!.rawUpdate('UPDATE $coinTable SET isRelevant = ? WHERE coinId = ?',
